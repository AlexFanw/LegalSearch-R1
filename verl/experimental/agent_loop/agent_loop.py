...
class AgentLoopWorker:
    ...
    def __init__(self, config: DictConfig, server_handles: list[ray.actor.ActorHandle]):
        ...
        run_sample_limit = 16
        self.sample_sema = asyncio.Semaphore(run_sample_limit)
    
    async def _run_with_sema(self, coro):
        async with self.sample_sema:
            return await coro
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        extra_info_batch = batch.non_tensor_batch.get("extra_info", [{}] * len(batch))
        ...
        for agent_name, messages, trajectory, tools_kwargs, extra_info in zip(agent_names, raw_prompts, trajectory_info, tools_kwargs_batch, extra_info_batch, strict=True):
            # tasks.append(
            #     asyncio.create_task(
            #         self._run_agent_loop(agent_name, messages.tolist(), sampling_params, tools_kwargs, trajectory, extra_info, batch.meta_info)
            #     )
            # )
            tasks.append(
                asyncio.create_task(
    ...
    async def _run_agent_loop(
        ...
        with rollout_trace_attr(
            ...
            output = await agent_loop.run(messages, sampling_params, tools_kwargs, extra_info, meta_info)
            return output
    ...
    def _postprocess(self, inputs: list[AgentLoopOutput]) -> DataProto:
        ...
        response_mask = [torch.tensor(x.response_mask, dtype=torch.float32) for x in inputs]
        response_mask_padded = pad_sequence(response_mask, batch_first=True)
        max_length = self.config.actor_rollout_ref.rollout.response_length
        if response_mask_padded.size(1) > max_length:
            response_mask_padded = response_mask_padded[:, :max_length]
        elif response_mask_padded.size(1) < max_length:
            pad_len = max_length - response_mask_padded.size(1)
            response_mask_padded = torch.nn.functional.pad(response_mask_padded, (0, pad_len), value=0.0)
        ...
        # 判断response_mask的shape，类型，以及response_mask包含的值有哪几种
        print("response_mask shape:", response_mask.shape)
        print("response_mask dtype:", response_mask.dtype)
        print("response_mask unique values:", torch.unique(response_mask))
