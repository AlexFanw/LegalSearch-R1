...
def compute_grpo_outcome_advantage(
    ...
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    token_entropy: Optional[torch.Tensor] = None,  # NEW: per-token entropy, (bs, T)
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    支持基于每个 token 熵的优势塑形（对所有 response token 生效）。

    Args:
        token_level_rewards: (bs, T_resp)
        response_mask:       (bs, T_resp) 0/1
        index:               (bs,) 组内 uid
        epsilon:             防止除零
        norm_adv_by_std_in_grpo: 是否按组内 std 归一化（原版 GRPO）
        config:              可提供以下可选项：
            - entropy_shaping_coeff: float = 0.4
            - entropy_shaping_max_frac: float = 0.5
            - apply_entropy_shaping: bool = True
        token_entropy:       (bs, T_resp) 每个 token 的熵

    Returns:
        advantages: (bs, T_resp)
        returns:    (bs, T_resp)
        metrics:    dict, 包含 "actor/shaping_term_to_adv_ratio"（熵塑形项与优势的比值均值）
    """
    # 每个样本一个 outcome 分数
    ...
    with torch.no_grad():
        ...
        for idx, lst in id2score.items():
            group_scores = torch.stack(lst)  # [k]
            if group_scores.numel() == 1:
                id2mean[idx] = torch.tensor(0.0, device=group_scores.device, dtype=group_scores.dtype)
                id2std[idx]  = torch.tensor(1.0, device=group_scores.device, dtype=group_scores.dtype)
            else:
                id2mean[idx] = group_scores.mean()
                id2std[idx]  = group_scores.std(unbiased=False)
    ...
    # 将样本级 advantage 展开到 token 维（只在 response token 上生效）
    advantages = scores.unsqueeze(-1) * response_mask  # (bs, T)
    ...
    if token_entropy is not None:
        ...
        # 形状检查（可选）
        if token_entropy.shape != advantages.shape:
            raise ValueError(f"token_entropy shape {token_entropy.shape} != advantages shape {advantages.shape}")

        abs_adv = advantages.abs()
        shaping_term = torch.minimum(coeff * token_entropy, max_frac * abs_adv)
        shaping_term = shaping_term * response_mask  # 只在 response token 生效
        ratio = shaping_term / (abs_adv + epsilon)
        mask = response_mask.to(dtype=advantages.dtype)
        per_sample_denom = mask.sum(dim=1).clamp_min(1.0)
        per_sample_ratio_mean = (ratio * mask).sum(dim=1) / per_sample_denom
        ratio_mean = per_sample_ratio_mean.mean().item()
        metrics["actor/shaping_term_to_adv_ratio"] = ratio_mean
        advantages = advantages + shaping_term.detach()  # 避免梯度回传到熵的分支
