...
python3 -m verl.trainer.main_ppo_user \
    ...
    trainer.total_training_steps=112 \
    trainer.val_before_train=True\
    trainer.validation_data_dir="$PROJECT_DIR/evaluate" \
    trainer.rollout_data_dir="$PROJECT_DIR/${timestamp}_rollout" \
    +trainer.plan_mask_weight=999 \
    reward_model.launch_reward_fn_async=True \
    data.train_files=$PROJECT_DIR/data/legalsearch_rag_train.parquet \
    data.val_files=$PROJECT_DIR/data/legalsearch_rag_test_sample16.parquet \
    actor_rollout_ref.rollout.multi_turn.tool_config_path="$PROJECT_DIR/user/deepresearch_tool_config_rag.yaml" \
    trainer.total_epochs=2 2>&1 | tee $PROJECT_DIR/log/${timestamp}_${project_name}_${experiment_name}.log