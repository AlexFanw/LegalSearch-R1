...
class WebSearchAgent:
    ...
    def scrape_and_check_valid(self, web_info: dict, user_query, search_query):
        ...
        if content is None:
            logger.info(f"访问URL为空：{web_info['link']}")
            return None
        
        if self.is_error_page(browser):
            logger.info(f"访问错误，抛弃URL：{web_info['link']}")
            return None

        # logger.info(f"成功访问URL：{web_info['link']}")
    ...
    def scrape_and_check_valid_api(self, url):
        ...
        if self.is_error_page(browser):
            # logger.error(f"Debug page_title: {browser.page_title}")
            # logger.error(f"Debug page_content:, {browser.page_content}")
            logger.info(f"访问错误，抛弃URL：{url}")
            return None
