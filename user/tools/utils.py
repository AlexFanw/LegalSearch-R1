...
def get_response_from_llm(
    ...
    try:
        # print(messages)
        # print(f"Calling LLM model: {model} with timeout {timeout}s")
        chat_endpoint = client.with_options(timeout=timeout).chat.completions
        ...
        print(f"LLM API error: {e}, model: {model}")
        ...
        if depth < 10:
            time.sleep(3)
            print(f"Retrying LLM API call, depth {depth + 1}...")
            print(f"Calling LLM model: {model} with timeout {timeout}s")
            return get_response_from_llm(messages=messages, client=client, model=model, stream=stream, temperature=temperature, depth=depth+1, timeout=timeout)
        raise e
